...
这些修改用于解决“App 预览与打印/相册保存的水平坐标不一致”：
...
当前状态：本地构建成功，10 项模拟器测试通过，但用户尚未对最新图片/纸条进行最终验收。因此不能 commit 或 push。

## 近期问题的根因

屏幕收据由 SwiftUI 排版，打印与相册由 `RasterRenderer` 手工绘制。此前两边使用不同的边距和列宽，造成 `Nothing fancy.`、任务文字、进度列在输出中偏移。第一次修复还曾将黑色文字绘制到黑条上，已经改回白字；这类视觉改动必须用实际输出验收。
