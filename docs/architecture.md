...
> 最后更新：2026-07-19。新会话开始时应先阅读仓库根目录的 `AGENTS.md`，再阅读本文、`context.md` 和 `decisions.md`。
...
- 空闲时底部为“保存到相册 / 打印到喵喵机 / 管理喵喵机”三个等宽按钮；打印中替换为全宽“取消打印”。
...
屏幕预览由 SwiftUI 实现，打印/相册由 Core Graphics 实现，两者不是同一棵视图树。必须共享并测试关键几何常量，否则会产生视觉漂移。当前关键常量是：

- 收据内容边距：18pt。
- 任务进度列宽：122pt。
- 详情列宽：52pt。
- 列间距：10pt。
- 任务文字起点：`margin + 32`（勾选框 22pt + 间距 10pt）。
- 口号黑条文字内边距：10pt。
- 口号按字体行高在黑条内垂直居中；任务名、详情、进度与勾选框共享 32pt 行中心。

已知技术债：屏幕与输出仍由两套布局代码绘制。以后调整 UI 时，必须同步检查 `RasterRenderer` 并用同一组布局常量或快照测试约束。
...
所有新功能必须先本地构建和测试，再由用户真机/真机打印验收；未验收不得 commit、push、打标签或发布。
